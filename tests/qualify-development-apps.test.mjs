...
const recipes = ["docling-app", "logseq-helper", "logseq-mcp", "zotero-controller", "zotero-desktop",
  "zotero-local-api-bridge", "zotero-automations", "zotero-mcp"];
const variables = ["DOCLING_CONTROLLER_IMAGE", "LOGSEQ_PROOF_HELPER_IMAGE", "LOGSEQ_PROOF_MCP_IMAGE",
...
async function fixture(t) {
  ...
  await Promise.all([mkdir(bin), mkdir(path.join(modules, "playwright"), { recursive: true }),
    mkdir(evidence, { recursive: true }), mkdir(path.join(root, "scripts"))]);
  ...
  const inventory = recipes.map((name) => ({
    name, dockerfile: "Dockerfile", context: ".", repository: `scholarserver-${name}`,
    manifest: "compose.yaml", service: "fixture", licenseCheck: "Dockerfile",
    nativeArchitectures: ["amd64", "arm64"], sourceInputs: ["Dockerfile", "source.txt"]
  }));
  await writeFile(path.join(root, "scripts/image-source-inventory.json"), JSON.stringify({ format: 1, recipes: inventory }));
  const images = await Promise.all(inventory.map(async (recipe, index) => ({
    recipe: recipe.name, target: `${registry}/${recipe.repository}:sha-${revision}-${architecture}`,
    localImageId: `sha256:${String(index + 1).repeat(64)}`, configDigest, architecture,
    rootfsDiffIds: [rootfs], sourceDigest: (await fingerprintRecipe(root, recipe)).digest
  })));
  ...
  await writeFile(path.join(modules, "playwright/index.cjs"), `
  ...
  `);
  await executable(path.join(bin, "python3"), shebang + `
    console.log([process.env.WRONG_IDENTITY || '${configDigest}', '${architecture}', '${rootfs}'].join('\t'));
  `);
  await executable(path.join(bin, "docker"), shebang + `
  ...
  `);
  await executable(path.join(bin, "node"), shebang + `
    const fs = require('node:fs');
    const path = require('node:path');
    const file = path.basename(process.argv[2] || '');
    if (/^check-(docling|logseq|zotero)-packaging.mjs$/.test(file)) {
      const names = ${JSON.stringify([...variables, "LOGSEQ_PROOF_HELPER_REVISION", "LOGSEQ_PROOF_MCP_REVISION",
        "LOGSEQ_PROOF_BROWSER_EXECUTABLE", "SCHOLARSERVER_BROWSER_EXECUTABLE", "SCHOLARSERVER_BROWSER_MODULES"])};
  ...
  `);
  const env = { ...process.env, PATH: `${bin}:${process.env.PATH}`, NATIVE_IMAGE_ROOT: root,
    GITHUB_ACTIONS: "true", RUNNER_ENVIRONMENT: "github-hosted", ARCH: architecture, REGISTRY: registry,
    REVISION: revision, DOCKER_HOST: "", DOCKER_CONTEXT: "", FIXTURE_RECEIPT: receipt,
    GATE_CALLS: path.join(root, "gate-calls.jsonl"), SCHOLARSERVER_BROWSER_MODULES: modules,
    SCHOLARSERVER_BROWSER_EXECUTABLE: browserExecutable };
  return {
    root, receipt, images, env, browserExecutable,
    run: (overrides = {}) => spawnSync("bash", [script], { env: { ...env, ...overrides }, encoding: "utf8", timeout: 15_000 }),
    calls: async () => (await readFile(env.GATE_CALLS, "utf8").catch(() => "")).trim().split("\n").filter(Boolean).map(JSON.parse),
    summaries: async () => {
      const directories = (await readdir(evidence)).filter((name) => name.includes(".development."));
      return (await Promise.all(directories.map((name) => readFile(path.join(evidence, name, "summary.json"), "utf8").catch(() => null))))
        .filter(Boolean).map(JSON.parse);
    }
  };
}
...
test("all three gates use receipt-bound IDs and the native executable, without writing release qualification", async (t) => {
  ...
  assert.deepEqual(calls.map((call) => call.file), ["check-docling-packaging.mjs", "check-logseq-packaging.mjs", "check-zotero-packaging.mjs"]);
  for (const call of calls) {
    for (const [index, variable] of variables.entries()) assert.equal(call.env[variable], proof.images[index].localImageId);
  ...
  assert.equal(summary.buildReceiptDigest, `sha256:${createHash("sha256").update(await readFile(proof.receipt)).digest("hex")}`);
...
test("workflow requires all gates before publication and only uploads selected synthetic evidence", async () => {
  ...
  assert.deepEqual(workflow.jobs.build.strategy.matrix.include, [
    { architecture: "amd64", runner: "ubuntu-24.04" }, { architecture: "arm64", runner: "ubuntu-24.04-arm" }
  ]);

...
try {
  ...
  assert.doesNotMatch(readerHtml, /themes\/ScholarServer|ss-reader-brand/, "new installations use native appearance");
  const branded = await fetch(`${origin}/api/appearance`, {
    method: "PUT",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ style: "scholarserver" })
  });
  assert.equal(branded.status, 200);
  const brandedHtml = await (await readerFetch("http://127.0.0.1:8082/i/")).text();
  assert.match(brandedHtml, /themes\/ScholarServer\/theme\.css/, "optional branded style remains available");
  assert.match(brandedHtml, /themes\/ScholarServer\/theme\.js/, "optional branding adapter remains available");

...
type Status = {
  ...
  variant: string; connectionMode: "complete-workspace" | "online-library";
...
const onlineStorageOptions: Array<{ value: StorageMode; title: string; detail: string }> = [
  { value: "metadata-only", title: "Citation data only", detail: "Use citations, collections, tags, and notes without downloading PDFs to this server." },
  { value: "zotero-storage", title: "Zotero Storage files on demand", detail: "Let ScholarServer fetch a PDF from Zotero Storage only when a tool or workflow needs it." }
];
...
function storageName(value: string | null) { return [...storageOptions, ...onlineStorageOptions].find((item) => item.value === value)?.title ?? value ?? "Not configured"; }
...
export function App() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setStatus(next);
      if ([...storageOptions, ...onlineStorageOptions].some((item) => item.value === next.storageMode)) setStorageMode(next.storageMode as StorageMode);
  ...
  "Attachment access settings were saved.", () => { setWebdavPassword(""); setSetupStage(status?.connectionMode === "online-library" ? "ready" : "authorization"); });
  const connectOnlineLibrary = () => run(
    () => request<Status>("account/online", { method: "POST", body: JSON.stringify({ apiKey: onlineApiKey }) }),
    "Your Zotero online library is connected.",
    () => { setOnlineApiKey(""); setSetupStage("storage"); }
  );
  ...
  return <div className="ss-app">
    ...
    <main className="ss-main">
      <div className="ss-page-heading"><div><h1>Zotero</h1><p>{online ? "Use your zotero.org library with ScholarServer and approved AI tools." : "Manage your private Zotero workspace, attachments, automations, and remote Zotero MCP."}</p></div>{status ? <span className={`ss-badge ${ready ? "ss-badge-success" : "ss-badge-warning"}`}>{ready ? "Ready" : "Setup needed"}</span> : null}</div>
      <nav className="ss-tabs" aria-label="Zotero sections">{visibleTabs.map((item) => <button key={item.id} className="ss-tab" aria-selected={tab === item.id} onClick={() => navigate(item.id)}>{item.label}</button>)}</nav>
      ...
      {status && tab === "overview" ? <div className="ss-stack">
        <div className="ss-grid ss-grid-3"><div className="ss-card"><div className="ss-metric-label">Connection</div><div className="ss-metric-value">{online ? "Online library" : "Complete workspace"}</div></div><div className="ss-card"><div className="ss-metric-label">Account</div><div className="ss-metric-value">{status.username ?? status.userId ?? "Not connected"}</div></div><div className="ss-card"><div className="ss-metric-label">Attachment access</div><div className="ss-metric-value">{storageName(status.storageMode)}</div></div></div>
        <section className="ss-card"><div className="ss-toolbar"><div><h2>Library connection</h2><p className="ss-card-description">{online ? "ScholarServer talks securely to your zotero.org library. Zotero Desktop is not installed on this server." : "Zotero runs privately on this server; ScholarServer talks to its supported local API."}</p></div><button className="ss-button ss-button-secondary" onClick={() => void refresh()}>Refresh</button></div><dl className="ss-details"><dt>Setup state</dt><dd>{status.state}</dd><dt>Connection</dt><dd>{online ? "Zotero Web API" : `Zotero Desktop ${status.version ?? ""}`}</dd>{online ? <><dt>Make changes</dt><dd>{status.permissions?.write ? "Allowed" : "Read only"}</dd><dt>Group libraries</dt><dd>{status.permissions?.groups ?? "None"}</dd></> : <><dt>Local API</dt><dd>{status.localApi}</dd><dt>File downloads</dt><dd>{status.downloadMode ?? "Not configured"}</dd></>}{status.storageMode === "linked-folder" ? <><dt>Shared folder</dt><dd>{status.linkedFolder ?? "/linked"}</dd><dt>ZotMoov automation</dt><dd>{status.linkedFolderAutomation ? "Enabled" : "Needs attention"}</dd></> : null}</dl></section>
        <section className="ss-card"><div className="ss-toolbar"><div><h2>{ready ? online ? "Online access" : "Synchronization" : "Setup is incomplete"}</h2><p className="ss-card-description">{ready ? online ? "Your tools read the latest library data directly from zotero.org; there is nothing to synchronize here." : "Start an immediate library sync when you need one." : online ? "Complete the guided account and attachment-access steps." : "Complete the guided account, storage, and authorization steps."}</p></div>{ready && !online ? <button className="ss-button" disabled={busy || status.syncInProgress} onClick={() => void run(() => request<Status>("sync", { method: "POST" }), "Zotero synchronization completed.")}>{busy || status.syncInProgress ? <span className="ss-spinner" /> : null}Sync now</button> : !ready ? <button className="ss-button" onClick={() => navigate("configuration")}>Continue setup</button> : null}</div></section>
      </div> : null}
      ...
      {status?.features.automations && tab === "automations" ? <AutomationsTab base={base} request={request} setNotice={setNotice} setError={setError} /> : null}

      {status && tab === "configuration" ? <div className="ss-stack">
        <SetupProgress stages={activeSetupStages} current={setupStage} />
        {setupStage === "account" ? <SetupPanel stage={1} total={online ? 3 : 4} title="Connect your Zotero account" description={online ? "Create a dedicated key in Zotero, then paste it here. ScholarServer stores it privately on this server." : "Sign in on Zotero's website. ScholarServer never receives your Zotero password."} next={status.accountConnected ? () => setSetupStage("storage") : undefined} nextLabel="Choose attachment access">
          {status.accountConnected ? <div className="ss-callout">Connected as <strong>{status.username ?? status.userId}</strong>. {online ? "ScholarServer uses a dedicated Zotero key that you can revoke at any time." : "Zotero stores its own account token."}</div> : online ? <div className="ss-stack"><div className="ss-callout"><strong>Create a ScholarServer key in Zotero.</strong> Allow library and notes access. Enable write access if you want AI tools to create or edit citations and notes.</div><div className="ss-form-actions"><a className="ss-button ss-button-secondary" href="https://www.zotero.org/settings/keys/new?name=ScholarServer&library_access=1&notes_access=1&write_access=1&all_groups=write" target="_blank" rel="noreferrer">Create key on Zotero</a></div><label className="ss-field">Zotero API key<input className="ss-input" type="password" autoComplete="off" value={onlineApiKey} onChange={(event) => setOnlineApiKey(event.target.value)} /><span className="ss-field-help">Copy the key Zotero shows after you save it. It is sent only to this ScholarServer.</span></label><div className="ss-form-actions"><button className="ss-button" disabled={busy || onlineApiKey.trim().length < 16} onClick={() => void connectOnlineLibrary()}>{busy ? <span className="ss-spinner" /> : null}Connect online library</button></div></div> : <div className="ss-form-actions"><button className="ss-button" disabled={busy || checkingAccount} onClick={() => void connectAccount()}>{busy || checkingAccount ? <span className="ss-spinner" /> : null}{checkingAccount ? "Waiting for approval…" : "Connect Zotero account"}</button>{authorizationUrl ? <a className="ss-button ss-button-secondary" href={authorizationUrl} target="_blank" rel="noreferrer">Open Zotero sign-in</a> : null}</div>}
        </SetupPanel> : null}

        {setupStage === "storage" ? <SetupPanel stage={2} total={online ? 3 : 4} title={online ? "Choose attachment access" : "Choose where attachments live"} description={online ? "Choose whether this server should access only citation data or fetch files from Zotero Storage when needed." : "Your references always synchronize through Zotero. Choose separately how this server obtains PDFs and other files."} back={() => setSetupStage("account")} next={() => void saveStorage()} nextLabel="Save and continue" nextDisabled={!status.accountConnected || (storageMode === "webdav" && (!webdavUrl || !webdavUsername || !webdavPassword))} busy={busy}>
          <label className="ss-field">{online ? "Attachment access" : "Storage option"}<select className="ss-input" value={storageMode} onChange={(event) => { const next = event.target.value as StorageMode; setStorageMode(next); setGroupFileSync(next === "zotero-storage"); }}>{(online ? onlineStorageOptions : storageOptions).map((option) => <option key={option.value} value={option.value}>{option.title}</option>)}</select><span className="ss-field-help ss-storage-description">{(online ? onlineStorageOptions : storageOptions).find((option) => option.value === storageMode)?.detail}</span></label>
        ...
        {setupStage === "ready" ? <SetupPanel stage={online ? 3 : 4} total={online ? 3 : 4} title="Zotero is connected" description={online ? "Your online library and attachment-access choice are ready." : "Your library, attachment choice, and ScholarServer authorization are ready."} back={() => setSetupStage(online ? "storage" : "authorization")}>
          <div className="ss-alert ss-alert-success">Setup is complete. {online ? "Approved AI tools can now work with your online Zotero library." : "You can synchronize Zotero and use approved AI tools from this server."}</div>
          <dl className="ss-details"><dt>Account</dt><dd>{status.username ?? status.userId ?? "Connected"}</dd><dt>Attachment access</dt><dd>{storageName(status.storageMode)}</dd>{online ? <><dt>Connection</dt><dd>Zotero Web API</dd><dt>Make changes</dt><dd>{status.permissions?.write ? "Allowed" : "Read only"}</dd></> : <><dt>Local API</dt><dd>{status.localApi}</dd></>}</dl>
        </SetupPanel> : null}
      </div> : null}
    </main>
  </div>;

...
function routeAutomationId(base: string): string | null {
  ...
  try {
    return decodeURIComponent(segments[position + 1]);
  } catch {
    return null;
  }
}
...
function statusFor(view: AutomationView) {
  if (view.configuration.runs.some((run) => run.state === "running"))
    return { label: "Running", className: "ss-badge-warning", filter: "active" as Filter };
  if (!view.configuration.active) return { label: "Inactive", className: "", filter: "inactive" as Filter };
  if (!view.readiness.ready)
    return { label: "Needs attention", className: "ss-badge-warning", filter: "attention" as Filter };
  if (view.configuration.enabled)
    return { label: "Scheduled", className: "ss-badge-success", filter: "active" as Filter };
...
export function AutomationsTab({
  ...
  useEffect(() => {
    ...
    setDraft((current) =>
      current?.id === selected.definition.id ? current : { id: selected.definition.id, value: updateFrom(selected) }
    );
  ...
  const saveValue = async (view: AutomationView, value: Update, message: string) => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Could not update the automation");
    } finally {
      setBusy(false);
    }
  };
  const activate = (view: AutomationView, active: boolean, value = updateFrom(view)) =>
    saveValue(
      view,
  ...
  const runNow = async (view: AutomationView) => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Could not start the automation");
    } finally {
      setBusy(false);
    }
  };
  ...
  if (selectedId && !selected)
    return (
      <section className="ss-card ss-empty-state">
        ...
        <button className="ss-button ss-button-secondary" onClick={() => navigate(null)}>
          Back to automations
        </button>
      </section>
    );
      ...
      <div className="ss-stack">
        <div className="ss-section-heading">
          <div>
            ...
            <p>Choose a ready-made Zotero workflow, activate it, then configure how and when it runs.</p>
          </div>
          <span className="ss-badge">{views.length} available</span>
        </div>
        <div className="ss-catalog-toolbar">
          <label className="ss-search-field">
            <Search size={17} />
            <span className="ss-visually-hidden">Search automations</span>
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search automations" />
          </label>
          <label className="ss-visually-hidden" htmlFor="automation-filter">
            Filter automations
          </label>
          <select
            id="automation-filter"
            className="ss-input ss-filter-select"
            value={filter}
        ...
        {visible.length ? (
          <div className="ss-automation-grid">
            {visible.map((view) => {
              const status = statusFor(view);
              const latest = view.configuration.runs[0];
              return (
                <article className="ss-automation-card" key={view.definition.id}>
                  <div className="ss-automation-card-top">
                    <span className="ss-automation-icon">
                      <Zap size={19} />
                    </span>
                    <span className={`ss-badge ${status.className}`}>{status.label}</span>
                  </div>
                  <div>
                    <p className="ss-eyebrow">{view.definition.category}</p>
                    <h3>{view.definition.name}</h3>
                    <p className="ss-card-description">{view.definition.description}</p>
                  </div>
                  <div className="ss-automation-meta">
                    <span>{view.definition.requires.map(dependencyName).join(", ")} required</span>
                    {latest ? <span>Last run {when(latest.startedAt)}</span> : <span>Never run</span>}
                  </div>
                  <div className="ss-automation-card-actions">
                    {view.configuration.active ? (
                      <button
                        className="ss-button ss-button-secondary"
                        disabled={busy}
                        onClick={() => void activate(view, false)}
                      >
                        Deactivate
                      </button>
                    ) : (
                      <button
                        className="ss-button ss-button-secondary"
                        disabled={busy}
                        onClick={() => void activate(view, true)}
                      >
                        Activate
                      </button>
                    )}
                    <button className="ss-button" onClick={() => navigate(view.definition.id)}>
                      View details
  ...
  return (
    <div className="ss-stack">
      ...
      <div className="ss-automation-detail-heading">
        <div className="ss-automation-icon ss-automation-icon-large">
          <Zap size={22} />
        </div>
        <div>
          <div className="ss-toolbar">
            <p className="ss-eyebrow">{selected.definition.category}</p>
            <span className={`ss-badge ${status.className}`}>{status.label}</span>
          </div>
          <h2>{selected.definition.name}</h2>
          <p>{selected.definition.description}</p>
          <div className="ss-dependency-list">
            {selected.definition.requires.map((dependency) => (
              <span key={dependency}>
                <CircleCheck size={14} />
      ...
      {!selected.readiness.ready ? (
        <div className="ss-callout ss-callout-warning">
          <strong>Dependency unavailable.</strong> {selected.readiness.message} Scheduled work is paused until it
          recovers.
        </div>
      ) : null}
        ...
        <div className="ss-toolbar">
          <div>
            ...
            <p className="ss-card-description">
              Activated automations can be configured and run. Activation does not start a schedule.
            </p>
          </div>
          <button
            className={`ss-button ${value.active ? "ss-button-secondary" : ""}`}
            disabled={busy}
            onClick={() => void activate(selected, !value.active, value)}
          >
            {value.active ? "Deactivate" : "Activate automation"}
          </button>
        </div>
      </section>
      ...
      <fieldset className="ss-automation-settings" disabled={!value.active || busy}>
        <section className="ss-card ss-stack">
          <div>
            <h3>Configuration</h3>
            <p className="ss-card-description">
              Choose which Zotero PDFs are processed and what happens to the result.
            </p>
          </div>
          <FolderPicker
            label="PDF folder"
            help="Choose a folder inside the shared attachment storage. Leave it at the root to include everything."
            value={config.folder}
            disabled={busy || !value.active}
            onChange={(folder) => setDraft({ id: draft.id, value: { ...value, configuration: { ...config, folder } } })}
            browse={browse}
          />
          <label className="ss-field">
            ...
            <input
              ...
              onChange={(event) =>
                setDraft({
                  id: draft.id,
                  value: { ...value, configuration: { ...config, limit: Number(event.target.value) } }
                })
              }
            />
            <span className="ss-field-help">Start small. Existing conversions with the same settings are reused.</span>
          </label>
          <label className="ss-check">
            <input
              ...
              checked={config.ocr}
              onChange={(event) =>
                setDraft({ id: draft.id, value: { ...value, configuration: { ...config, ocr: event.target.checked } } })
              }
            />
            <span>
              <strong>Use OCR</strong>
              <small>Enable for scanned or image-only PDFs; it requires considerably more processing power.</small>
            </span>
          </label>
          <label className="ss-check">
            <input
              type="checkbox"
              checked={config.attachMarkdown}
              onChange={(event) =>
                setDraft({
                  id: draft.id,
                  value: { ...value, configuration: { ...config, attachMarkdown: event.target.checked } }
                })
              }
            />
            <span>
              <strong>Attach Markdown to Zotero</strong>
              <small>Add the converted Markdown beside its source PDF in the Zotero item.</small>
            </span>
          </label>
        </section>
        <section className="ss-card ss-stack">
          <div className="ss-toolbar">
            <div>
              <h3>Schedule</h3>
              <p className="ss-card-description">
                Leave automatic runs off and use Run now, or choose a recurring interval.
              </p>
            </div>
            <Clock3 size={20} className="ss-muted" />
          </div>
          <label className="ss-check">
            <input
              type="checkbox"
              checked={value.enabled}
              onChange={(event) => setDraft({ id: draft.id, value: { ...value, enabled: event.target.checked } })}
            />
            <span>
              <strong>Run automatically</strong>
              <small>The schedule pauses whenever Docling is unavailable.</small>
            </span>
          </label>
          {value.enabled ? (
            <label className="ss-field">
              ...
              <select
                ...
                onChange={(event) =>
                  setDraft({ id: draft.id, value: { ...value, intervalMinutes: Number(event.target.value) } })
                }
              >
          ...
          <div className="ss-form-actions">
            <button
              className="ss-button ss-button-secondary"
              disabled={busy}
              onClick={() => void saveValue(selected, value, "Automation settings were saved.")}
            >
              Save settings
            </button>
            <button
              className="ss-button"
              disabled={busy || !selected.readiness.ready}
      ...
      {!value.active ? (
        <div className="ss-callout">
          <strong>This automation is inactive.</strong> Activate it to configure or run it.
        </div>
      ) : null}
      ...
      <section className="ss-card">
        ...
        {selected.configuration.runs.length === 0 ? (
          <p className="ss-muted">This automation has not run yet.</p>
        ) : (
          <div className="ss-run-list">
            {selected.configuration.runs.map((run) => (
              <div className="ss-run" key={run.id}>
                <div>
                  <span
                    className={`ss-badge ${run.state === "succeeded" ? "ss-badge-success" : run.state === "failed" ? "ss-badge-error" : "ss-badge-warning"}`}
                  >
                    {run.state}
                  </span>
                  <strong>{run.trigger === "scheduled" ? "Scheduled run" : "Manual run"}</strong>
                  <small>{when(run.startedAt)}</small>
                </div>
                {run.summary ? (
                  <p>
                    {run.summary.discovered ?? 0} found · {run.summary.converted ?? 0} converted ·{" "}
                    {run.summary.attached ?? 0} attached
                  </p>
                ) : null}

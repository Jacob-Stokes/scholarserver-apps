...
export class ZoteroClient {
  constructor(private connection: ZoteroConnection | (() => Promise<ZoteroConnection>)) {}
  ...
  private async req<T = any>(
    ...
    const serverId =
      !connection.local || method === "GET" || method === "HEAD"
        ? null
        : await this.serverId(baseUrl, connection.headers ?? {});
  ...
  put<T = any>(path: string, body: any, headers: Record<string, string> = {}) {
    return this.req<T>("PUT", path, body, headers);
  }
  patch<T = any>(path: string, body: any, headers: Record<string, string> = {}) {
    return this.req<T>("PATCH", path, body, headers);
  }
  delete<T = any>(path: string, headers: Record<string, string> = {}) {
    return this.req<T>("DELETE", path, undefined, headers);
  }
}

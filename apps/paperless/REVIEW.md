...
## Resumed source checkpoint — 22 September 2026

The prior source-only draft has been restored to the current apps repository as
an undistributed workspace. The old development Dockerfile is outside the native
image inventory; no package manifest, release image, catalog entry or installed
application was created. The independently built Access screen now uses the
shared read-resource lifecycle, with its existing draft and fail-closed sign-in
behavior. The root workspace and UI lockfiles were reconciled without broadening
runtime permissions.

The full repository `npm test`, Paperless UI typecheck/build and nine MCP tests
passed on 22 September. `node --test tests/read-lifecycle-contract.test.mjs`
also passed. The browser Access check was not run: its optional Playwright module
path is not configured in this workspace. These are source and mock checks, not a
new native probe or fresh installation. The older 3.1.3 probe evidence below is
historical; upstream 3.2.1 must be reviewed and qualified before release.

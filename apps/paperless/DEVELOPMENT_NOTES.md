...
## Draft resumed in the current apps tree — 22 September 2026

The earlier `codex/paperless-draft` source and its native probe were brought into
the current apps tree. The existing incomplete root-workspace entries now point to
this draft's actual integration and UI workspaces, so repository checks exercise
its read-only MCP code instead of failing on a missing directory. Its separate UI
lockfile pins the shared presentation dependencies. The native recipe remains in
`development/PaperlessMcp.dockerfile`: it is deliberately not in the distributable
image inventory while the package contract, onboarding, backup and current
upstream-version gates remain open. No `package/` manifest exists, so catalog
discovery cannot present this draft as installable.

The old native evidence used Paperless 3.1.3. Upstream lists 3.2.1 as its latest
release on this date; that is research, not a version change or compatibility
qualification. The focused read-only MCP tests, UI typecheck/build and repository
source pretest pass. Native 3.2.1 startup, current ACL/ingestion/restore, image
review, Manager installation and live acceptance have not been repeated.

...
function runCli(args, stdin = null) {
  return new Promise((resolve, reject) => {
    ...
    process.once("exit", (code) =>
      code === 0 ? resolve(output) : reject(new Error(output.trim() || `LiveSync CLI exited ${code}`))
    );
  });
}
...
async function start(config) {
  ...
  child = spawn(
    "node",
    ["/app/dist/index.cjs", database, "--settings", settingsPath, "--vault", vault, "--interval", "30", "daemon"],
    {
...
async function reconcile() {
  ...
  if (!child && Date.now() >= retryAfter) {
    try {
      ...
      await update({
        state: "error",
        lastError: error instanceof Error ? error.message.slice(0, 1000) : "LiveSync failed"
      });
    }
  }
}

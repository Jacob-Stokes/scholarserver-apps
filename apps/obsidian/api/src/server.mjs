...
app.get("/files", (c) => {
    ...
    if (!root)
        return c.json({ error: "Invalid dir" }, 400);
...
app.get("/files/*", (c) => {
    ...
    if (stat.isDirectory())
        return c.json({ error: "Path is a directory — use /folders/*" }, 400);
...
app.post("/bulk/frontmatter", async (c) => {
    ...
    if (!Array.isArray(body.patches))
        return c.json({ error: "patches array required" }, 400);
    if (body.patches.length > MAX_BULK)
        return c.json({ error: `max ${MAX_BULK} patches per request` }, 400);
    const results = await Promise.all(body.patches.map(async (p) => {
        if (!p || typeof p !== "object")
            return { path: String(p), error: "invalid patch" };
        const rec = p;
        const path_ = typeof rec.path === "string" ? rec.path : "";
        if (!path_)
            return { path: path_, error: "path required" };
        const fields = rec.fields;
        if (!fields || typeof fields !== "object")
            return { path: path_, error: "fields object required" };
        ...
        if (!abs)
            return { path: path_, error: "invalid path" };
        if (!fs.existsSync(abs))
            return { path: path_, error: "not found" };
        try {
 ...
 */
function mergeFrontmatter(original, patch) {
    const fenceStart = "---\n";
    const closingFence = "\n---";
    let fmBlock; // everything between the opening ---\n and the closing \n---
    let rest; // everything after the closing fence (incl. the trailing \n)
    let hadFrontmatter;
...
app.get("/folders/*", (c) => {
    ...
    const stat = fs.statSync(resolved);
    if (!stat.isDirectory())
        return c.json({ error: "Not a folder" }, 400);
    const children = [];
...
app.put("/folders/*", (c) => {
    ...
    if (existed) {
        const stat = fs.statSync(resolved);
        if (!stat.isDirectory())
            return c.json({ error: "Path exists and is not a folder" }, 409);
    }
...
app.delete("/folders/*", (c) => {
    ...
    if (!resolved)
        return c.json({ error: "Invalid path" }, 400);
    if (resolved === path.resolve(VAULT_PATH))
        return c.json({ error: "Refusing to delete vault root" }, 400);
    if (!fs.existsSync(resolved))
        return c.json({ error: "Folder not found" }, 404);
    const stat = fs.statSync(resolved);
    if (!stat.isDirectory())
    ...
    if (recursive) {
        ...
        if (entries.length > 0)
            return c.json({ error: "Folder not empty — pass ?recursive=true" }, 409);
...
app.get("/search", (c) => {
    ...
    if (!query)
        return c.json({ error: "q param required" }, 400);
    ...
    for (const file of files) {
        ...
        if (hit.length > 0)
            results.push({ path: file, matches: hit.slice(0, 5) });
    }
...
app.get("/daily", (c) => {
    ...
    if (!abs || !fs.existsSync(abs))
        return c.json({ path: rel, exists: false }, 404);
...
app.post("/bulk/write", async (c) => {
    ...
    if (body.files.length > MAX_BULK)
        return c.json({ error: `max ${MAX_BULK} files per request` }, 400);
    const results = [];
...
app.post("/bulk/delete", async (c) => {
    ...
    if (body.paths.length > MAX_BULK)
        return c.json({ error: `max ${MAX_BULK} paths per request` }, 400);
...
app.post("/bulk/read", async (c) => {
    ...
    const results = await Promise.all(body.paths.map(async (p) => {
        if (typeof p !== "string")
            return { path: String(p), error: "invalid path" };
        const abs = resolvePath(p);
        if (!abs)
        ...
        try {
            const stat = fs.statSync(abs);
            if (stat.isDirectory())
                return { path: p, error: "is a directory" };
            if (format === "base64") {
 ...
 * `\n---` on its own line.
 */
function sliceFrontmatter(full) {
    if (!full.startsWith("---\n"))
        return "";

...
if (!TOKEN) {
  console.error("MCP_BEARER_TOKEN required");
  process.exit(1);
}
...
async function main() {
  const transport = new SSEClientTransport(new URL(`${MCP_URL}/sse`), {
    ...
    eventSourceInit: {
      fetch: (u, init) =>
        fetch(u, { ...init, headers: { ...(init?.headers as any), Authorization: `Bearer ${TOKEN}` } })
    }
  });
  ...
  const search = await client.callTool({
    ...
    arguments: { q: "homelab", path: "Homelab", max_results: 3 }
  });

...
export const ATTACHMENTS_TOOL = {
  ...
  description:
    "List, read, upload, move or safely delete non-Markdown vault attachments. Binary content uses base64 and is size-limited. Writes refuse overwrite by default; deletes move to .trash by default.",
    ...
    const all = (Array.isArray(result?.files) ? result.files : [])
      .filter(
        (candidate: unknown): candidate is string =>
          typeof candidate === "string" && !candidate.toLowerCase().endsWith(".md")
      )

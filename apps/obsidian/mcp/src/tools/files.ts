...
export const FilesInput = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    moves: z
      .array(z.object({ from: FilePath, to: FilePath }))
      .min(1)
      .max(50),
...
export const FILES_TOOL = {
  name: "obsidian_files",
  description:
    "DEPRECATED compatibility tool for older clients. Use obsidian_get_note, obsidian_write_note, obsidian_move_note, obsidian_delete_note and obsidian_bulk. Deletes now move notes to .trash rather than permanently removing them.",
...
export async function handleFiles(ctx: ToolContext, input: FilesInput) {
  switch (input.action) {
    case "read":
      return handleGetNote(ctx, { path: input.path, view: "full", occurrence: 1 });
    case "write":
      return handleWriteNote(ctx, {
        path: input.path,
    ...
    case "delete":
      return handleDeleteNote(ctx, { path: input.path, mode: "trash", dry_run: false });
    case "move":
      return handleMoveNote(ctx, {
        from: input.from,
        to: input.to,

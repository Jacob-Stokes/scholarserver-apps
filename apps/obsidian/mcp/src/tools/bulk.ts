import { z } from "zod";
import { runBounded, sha256, type ToolContext } from "../lib/vault.js";
import { handleFrontmatter } from "./metadata.js";
import { handleAppendNote, handleDeleteNote, handleMoveNote, handleWriteNote } from "./notes.js";
...
export const BulkInput = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    files: z
      .array(z.object({ path: Path, content: z.string(), expected_hash: Hash }))
      .min(1)
      .max(50),
  ...
  z.object({
    ...
    entries: z
      .array(z.object({ path: Path, content: z.string().min(1), heading: z.string().optional(), expected_hash: Hash }))
      .min(1)
      .max(50),
  ...
  z.object({
    ...
    moves: z
      .array(z.object({ from: Path, to: Path, expected_hash: Hash }))
      .min(1)
      .max(50),
  ...
  z.object({
    ...
    patches: z
      .array(z.object({ path: Path, fields: z.record(z.unknown()), expected_hash: Hash }))
      .min(1)
      .max(50),
    dry_run: z.boolean().default(false)
  })
...
export const BULK_TOOL = {
  name: "obsidian_bulk",
  description:
    "Run bounded batches of note reads, writes, appends, moves, safe deletes, or frontmatter merges. Each mutation reports partial failures; batches are not atomic. Defaults remain create-only, trash-not-delete, and no destination overwrite.",

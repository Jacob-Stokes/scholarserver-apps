...
test("the integration image ships the Manager connection module used by setup and research", async () => {
  ...
  assert.match(recipe, /COPY apps\/n8n\/integration\/schedule-management\.mjs \.\//);
});

test("native Manager APIs participate in the integration image source fingerprint", async () => {
  ...
  assert.ok(recipe.sourceInputs.includes("apps/n8n/integration/schedule-management.mjs"));
});
